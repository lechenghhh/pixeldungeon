...
public class Armor extends EquipableItem {
	...
	private static final String TXT_EQUIP_CURSED	= "你的%s在你周围痛苦地收缩";

	private static final String TXT_IDENTIFY	= "您现在已经足够熟悉%s以识别它。它是%s。";

	private static final String TXT_TO_STRING	= "%s :%d";
	private static final String TXT_BROKEN		= "broken %s :%d";

	private static final String TXT_INCOMPATIBLE =
		"不同魔法的交互作用已经擦除了这个盔甲上的雕文！";
	...
	public String info() {
		...
		if (levelKnown) {
			...
			if (STR > Dungeon.hero.STR()) {
				...
				if (isEquipped( Dungeon.hero )) {
					info.append(
						"\n\n由于你的力量不足，你的移动速度和防御技能会降低。 " );
				} else {
					info.append(
						"\n\n因为你的力量不足，穿上这件盔甲会降低你的移动速度和防御技能。 " );
				}

			}
		} else {
			info.append(
				"\n\n典型的 " + name + " 提供高达 " + typicalDR() + " 攻击力 " +
				" and requires " + typicalSTR() + " 力量点. " );
		...
		if (isEquipped( Dungeon.hero )) {
			info.append( "\n\n你穿着 " + name +
				(cursed ? ", 因为它是被诅咒的，你无力移除它。" : ".") );
		} else {
			if (cursedKnown && cursed) {
				info.append( "\n\n你能感觉到一种邪恶的魔法潜伏在 " + name + "." );
			}
		}
